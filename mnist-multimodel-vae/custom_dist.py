...
class MixtureOfDiagNormals(TorchDistribution):
    ...
    def log_prob(self, value):
        ...
        # result = self.probs * torch.exp(-eps_sqr + eps_sqr_min.unsqueeze(-1)) / (
        #    coord_scale_prod)  # L B K
        # result = torch.log(result.sum(-1))  # L B

        coord_scale_prod = self.coord_scale.log().sum(-1)
        result = self.log_probs + (-eps_sqr + eps_sqr_min.unsqueeze(-1)) - coord_scale_prod
        result = torch.logsumexp(result, dim=-1)

        if torch.sum(torch.isnan(result)) > 0 or torch.sum(result) == 0:
            breakpoint()
